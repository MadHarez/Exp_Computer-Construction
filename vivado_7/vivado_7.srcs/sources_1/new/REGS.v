`timescale 1ns / 1ps
module REGS(R_Data_A,R_Data_B,W_Data,R_Addr_A,R_Addr_B,W_Addr,Write_Reg,rst,clk,input_val,Write_val_btn);
	input clk;//写入时钟信号
    input rst;//清零信号
...
//    end
    always@(posedge clk or posedge rst) begin
        if(rst) begin for(i=0;i<32;i=i+1) REG_Files[i]<=0;end
        else begin
         if(Write_Reg) REG_Files[W_Addr]<=W_Data;
         else if(Write_val_btn) REG_Files[W_Addr]<=input_val;
        end
    end
